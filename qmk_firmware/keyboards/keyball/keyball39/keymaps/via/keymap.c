...
/* ==========================================================================
   1. 独自キーコードの定義 (AML_TOなど)
   ========================================================================== */
enum custom_keycodes {
  AML_TO = SAFE_RANGE, // オートマウス有効/無効切り替え
  AML_I50,             // タイムアウト時間を増やす
  AML_D50,             // タイムアウト時間を減らす
  SCRL_DVI,            // スクロール速度調整
  SCRL_DVD,
  CPI_D1K,             // CPI調整
  CPI_D100,
...
bool aml_active = false;      // 現在マウスレイヤーか
bool aml_enable_sw = true;    // オートマウス機能自体のON/OFF
...
/* ==========================================================================
   3. キーマップ定義 (ご提示の universal レイアウトを完全維持)
   ========================================================================== */
...
// ボールの動きを検知してレイヤーを切り替える
void report_mouse_user(report_mouse_t* mouse_report) {
    if (!aml_enable_sw) return; // AML_TOキーでOFFにされている場合は無視
...
// キー操作の処理（AML_TOやクリック離脱の処理）
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    switch (keycode) {
        case AML_TO: // 自動マウスレイヤーをトグル
            if (record->event.pressed) {
                aml_enable_sw = !aml_enable_sw;
            }
            return false;
        case KC_MS_BTN1 ... KC_MS_BTN5: // マウスボタン
            if (aml_active && !record->event.pressed) {
                // ボタンを離した瞬間、タイムアウトを短い時間（0.5秒）に変更
